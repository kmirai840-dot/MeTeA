-- PostgreSQL schema version 1. UTC timestamps retain the existing TEXT format.
...
CREATE TABLE IF NOT EXISTS user_general_activities (
    ...
    activity_type TEXT NOT NULL,
    title TEXT NOT NULL,
    occurred_at TEXT NOT NULL DEFAULT to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS'),
...
CREATE UNIQUE INDEX ux_users_email ON users(lower(email)) WHERE email IS NOT NULL AND TRIM(email) <> '';
CREATE UNIQUE INDEX ux_users_auth_identity ON users(auth_provider, auth_subject)
