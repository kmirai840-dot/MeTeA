...
- 選考スケジュールの「次の予定・期限」セルから、同一画面内の「選考予定・結果を登録」ダイアログを開く。
...
- 選考準備はダイアログへ統合せず、選考スケジュールの企業行から独立画面を同一タブで開く。
...
- TOP画面の主機能カードは「① 自分を知る」「② 求人を比較する」「③ 応募後を管理する」「④ 活動を振り返る」とする。「④ 活動を振り返る」は `assets/analytics.svg` を使用し、就職活動ダッシュボードへ遷移する。設定はTOP画面上部のナビゲーションから開く。
