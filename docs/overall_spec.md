...
> 2026-09-13 追補：描画変更を見送り、取得方法と保存時連携を追加改善。職歴・見直し・比較・詳細・応募・選考準備・レポート・運営者を対象とし、UIと評価算式を維持する。

> 2026-09-13 性能方針：火曜日の限定リリースはStreamlitを維持し、重複取得と接続回数を削減。[今回のAs-Is / To-Be・計測・将来案](streamlit_release_performance.md)を参照。
...
As-Is：利用者へ修正状況を案内する掲載欄がなかった。
To-Be：ホーム右列の「次の一歩」の前に「お知らせ」を追加。日付・見出し・概要を常時表示し、修正詳細と利用者の操作案内はdetails/summaryで展開する。2026-09-13のAI評価・求人一覧エラー、次の一歩、保存速度改善を掲載。保存済み求人の再登録は不要、失敗表示が残るときは再試行1回、再失敗は運営者へ連絡と案内する。最長3秒達成は掲載しない。

app.py内の静的掲載で、AI・DB問い合わせを追加しない。利用者別の求人・入力内容・評価点・秘密情報は掲載しない。今後の更新はコード編集と公開反映で行う。管理画面・既読管理は今回追加しない。390px/1440pxのブラウザ表示で展開操作と欄内横あふれなしを確認。

### お知らせの配置変更（2026-09-13）

利用者指定の上側赤枠に合わせ、掲載欄をホーム右列からヘッダーナビゲーションへ移動。「お知らせ」を押すと本文パネルが開き、もう一度押すと閉じる。本文と詳細展開は維持。右列は「次の一歩」から開始する。パネルは画面幅に収め、長い本文はパネル内で縦スクロールする。スマートフォンではヘッダー内の間隔を調整する。
...
全画面の入力処理を監査し、フォームの一括送信と画面内更新を適用。数値評価と保存済み意味判定を分離して整合性を維持。
