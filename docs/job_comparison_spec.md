...
2026-09-13：一覧の求人・評価・応募判断・紹介経路を1操作で取得。紹介経路は絞り込みとカードで共用し、詳細の確認記録も重複取得しない。UI・採点ルールは維持。[As-Is / To-Beと実測](streamlit_release_performance.md)。
...
## 2026-09-13 許容と確認結果の責任分離

| 対象 | As-Is | To-Be |
|---|---|---|
| 本人判断 | 確認不要のみ | 許容チェック＋点数への反映（そのまま／加点／減点） |
| 事実と判断 | 確認結果と確認不要が排他的 | 確認結果・許容・本人調整を別フィールドで保存。同時に入力可能 |
| 採点 | AI評価のみ | AI評価を保持し、求人詳細の総合点の下へ本人調整と調整後を併記 |
| 許容項目 | 要確認に残る | 評価一覧は「許容」、次に確認すること・未確認カードから除外。確認済み・許容した項目で再編集 |
| 円グラフ下の小さな内訳 | 確定軸25点分等を表示 | ユーザー依頼により削除。円グラフ・評価一覧は維持 |

本人調整は許容時のみ各項目±1点、合計±5点、調整後0〜100点。名前が同じ記録は二重加点しない。求人一覧・比較のAIスコアや円グラフはAI素点のまま。確認結果の変更・取消はAI再評価、本人判断だけの変更はAI再評価なし。許容の取消で確認結果は消さず、確認結果の取消で許容は消さない。

同じカード内、従来の背景・枠・配色・区切りと最下部の一括保存を維持。以前の確認不要は自動的に許容に変換せず「以前に確認不要にした項目」で本人が改めて選択する。

DB：既存user_job_confirmation_resolutionsへaccepted（INTEGER、既定0）とscore_adjustment（INTEGER、既定0）を追加。result_text/statusは事実側、追加列は本人判断側。保存は一つのトランザクション、所有者チェックを維持。SQL/SQLite初期DDLと既存DB向け追加移行を更新。
...
保存済みAI意味判定と最新の数値条件を再合成し、一覧・詳細で同じ評価を使用する。年収の上限・原文の想定年収も考慮する。

As-Is / To-Be・対象一覧・検証範囲は [入力処理の監査記録](input_update_audit.md) を参照。


## 2026-09-13：比較・詳細の取得範囲

As-Is：比較は全求人・全評価と求人ごとの通勤確認を取得。詳細は全求人分の評価・応募判断を取得していた。
To-Be：比較は選択した2〜3件の本人の求人・評価・通勤を一括取得。詳細は指定求人の評価・応募判断だけを取得し、確認記録は初期表示で共用する。空の選択・他人のID・削除済みの求人から全件に広げない。監視も比較対象だけを読み、完了時の自動反映は維持する。
表示内容・カード・グラフ・評価算式は変更しない。詳細の遅延描画は今回見送り。[性能改善](streamlit_release_performance.md)を参照。
