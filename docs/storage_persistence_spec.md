...
## 2026-09-13 使用できるツール・スキルの独立登録

As-Is：スキルを登録する専用欄がなく、職務経歴の業務内容に記載する必要があった。To-Be：職務経歴・スキル画面と登録内容の見直しに任意の独立入力欄を追加。職歴なしでも自己申告の機能・経験を保存できる。入力中は送信せず、保存ボタンで本人のuser_skills表へ永続保存する（最大3,000文字、空欄保存で削除相当）。

AI入力のself_reported_tools_and_skillsへ渡し、学習・個人活動を実務経験と区別する。空欄から能力不足を断定しない既存方針を維持。変更時だけ本人の求人評価を更新対象とし、同値保存は再評価しない。運営者エクスポート対象にも追加。既存DBにはtools/add_user_skills_table.pyで追加DDLを適用してから公開する。既存行は変更しない。
...
## 2026-09-13：取得・保存操作の境界

As-Is：画面内で同じ本人情報を複数接続で取得し、応募判断と応募管理の作成が別々に確定していた。
To-Be：画面の初期取得をサービス単位でまとめ、同じ操作中だけ接続・有効性確認を共用する。描画前に接続を返す。応募判断と関連する応募作成を一括で確定し、失敗時は全体を戻す。個別取得の所有者条件・論理削除条件は維持し、IDの空リストは0件とする。
DBスキーマ・保存先・既存入力データ・認証方式は変更しない。詳細：[性能改善](streamlit_release_performance.md)。
